...
contains
  ...
  !> If @a xShift is sepcified, that value us subtracted from @x
  ...
  subroutine SensorGradient (cParam,sGrad,ierr)
    ...
    sGrad = 0.0_dp
    if (.not.associated(cParam%engine)) then
       nullify(sensor)
    else if (size(cParam%engine%args) < 1) then
       nullify(sensor)
    else
       argPtr => cParam%engine%args(1)
       sensor => argPtr%p
    end if
    ...
    ierr = 0
    xArg = sensor%value
    if (associated(cParam%engine)) then
       eGrad = EngineRate(cParam%engine,ierr,xArg)
    else
       eGrad = 1.0_dp
    end if
  ...
  recursive subroutine UpdateSensor (sensor,ierr)
    ...
    case (SPRING_AXIAL_p, SPRING_JOINT_p)
       ...
       case default
          ...
          call updateSpringBase (springs(sensor%p%index),ierr)
       end select
    ...
    case (DAMPER_AXIAL_p, DAMPER_JOINT_p)
       ...
       case default
          ...
          call updateDamperBase (dampers(sensor%p%index),ierr)
       end select
    ...
    case (TRIAD_p)
       ...
       case (LOCAL_p)
          ...
          sensor%p%value = GetLocal(triads(sensor%p%index),dof,sensor%p%entity)
       case default
          ...
          case (POS_p)
             if (dof >= 4 .and. dof <= 9) then
                ...
                sensor%p%value = GetAngle(triads(sensor%p%index),dof-3)
             end if
          case (FORCE_p)
             !! Evaluate global force component of the triad
             sensor%p%value = GetGlobalForce(triads(sensor%p%index),dof)
          case (W_SPEED_p)
             !! Evaluate the wind speed at the triad location
             call getWindSpeed (triads(sensor%p%index)%ur(:,4),ourTime%value, &
          ...
          case (F_VEL_p:DYN_P_p)
             ...
             call getSeaState (ourEnvir,ourTime%value, &
                  &            triads(sensor%p%index)%ur(:,4),work,ierr)
             if (ierr < 0) ierr = lerr + ierr
             select case (sensor%p%entity)
    ...
    case (RELATIVE_TRIAD_p)
       ...
       sensor%p%value = GetRelative(triads(sensor%p%index), &
            &                       triads(sensor%p%index2), &
            &                       sensor%p%dof,sensor%p%entity,sensor%p%value)
