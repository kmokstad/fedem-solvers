...
contains
  ...
  subroutine BuildStructControlJacobi(pCS,sys,sam)
    ...
    integer  :: iNode, iCin, iCout, iStart, iEnd, nPerturb, perturbAlgNo
    real(dp) :: eGrad
...
!    if (pCS%ctrlSysEigFlag == 1) then !! nPertub = 3: P, I and D gains
!       nPerturb = 3
!       call EstimateControllerProperties01(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 2) then
!       nPerturb = 4
!       call EstimateControllerProperties02(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 3) then
!       nPerturb = 6
!       call EstimateControllerProperties03(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 4) then !! nStep = 1
!       nPerturb = 5
!       call EstimateControllerProperties04(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 5) then !! nStep = 2
!       nPerturb = 5
!       call EstimateControllerProperties05(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 6) then !! nStep = 3
!       nPerturb = 5
!       call EstimateControllerProperties06(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 7) then !! nStep = 4
!       nPerturb = 5
!       call EstimateControllerProperties07(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 8) then !! nStep = 5
!       nPerturb = 5
!       call EstimateControllerProperties08(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 9) then !! nStep = 10
!       nPerturb = 5
!       call EstimateControllerProperties09(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 10) then !! nStep = 100
!       nPerturb = 5
!       call EstimateControllerProperties10(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 11) then !! nStep = 1000
!       nPerturb = 5
!       call EstimateControllerProperties11(sys,pCS%pMech, pCS%pControl,sam%mpar,&
!       &                                 pCS%numVregIn, pCS%whichVregIn,      &
!       &                                 pCS%numVregOut, pCS%whichVregOut,    &
!       &                                 nPerturb, pCS%ctrlProps, ierr)
!    else if (pCS%ctrlSysEigFlag == 500) then
!       nPerturb = 1
...
subroutine EstimateControllerProperties03(sys,mech,ctrl,msim,       &
  ...
  subroutine EstimateControllerProperties04(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
    ...
    real(dp) :: dt0                              !! quazi-initial time step
    real(dp) :: dy0                              !! quazi-initial input perturbation, dy0 = 0
    real(dp) :: nStep                            !! number of steps in between perturbations
    ...
    do i = 1, numVregIn
       do j = 1, nPerturb
          ...
          call CopyControlType(ctrlCopy,ctrlCopyCopy)
          !! First perturbation
          ddt = (1.0_dp/nStep)*dt(j)
          ddy = (1.0_dp/nStep)*dy(j)
          do ii = 1, nStep
  ...
  subroutine EstimateControllerProperties05(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties05

    subroutine EstimateControllerProperties06(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties06

    subroutine EstimateControllerProperties07(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties07

    subroutine EstimateControllerProperties08(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties08

    subroutine EstimateControllerProperties09(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties09

    subroutine EstimateControllerProperties10(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties10

    subroutine EstimateControllerProperties11(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
  ...
  end subroutine EstimateControllerProperties11
  ...
  subroutine EstimateControllerProperties500(sys,mech,ctrl,msim,       &
    ...
    do i = 1, numVregIn
       ...
       !! Calculate controller properties
       do j = 1, nPerturb
          duTable(j,:) = (1.0_dp/dyMatrix(j,1)) * duTable(j,:)
       end do
 ...
 !! Backup of old subroutine
  subroutine EstimateCtrlSysProp_old(sys,mech,ctrl,msim,      &
    &                                        numVregIn,  whichVregIn,  &
    &                                        numVregOut, whichVregOut,      &
    &                                        nPerturb, ctrlSysGains, ierr)
    ...
    !! Use a perturbation method, simular to the Matrix Stiffness Method /
    !! (Virtual) Displacement Method / Unit Load Method to find
    !! the properties of the control system. These control system properties
    !! will be added to existing matrices when conducting
    !! modal analysis / eigenvalue analysis.
    !! The values of interest are:
    !! du/de: Change du in output from control system with respect to
    !!        change de in input to control system.
    !!        du/de = proportional gain, Kp
    !! du/(de/dt): Change du in output from control system with respect to
    !!             change de/dt in input to control system.
    !!             du/(de/dt) = derivative gain, Kd
    !! du/(int(de)dt): Change du in output from control system with respect to
    !!                 change int(de)dt in input to control system.
    !!                 du/(int(de)dt) = integral gain, Ki
    !!
    !! Working order:
    !! 1) Get initial values u(e0) for the control system.
    !! 2) Change the input e for the control system from e0 to e = e0+de,
    !!    where de is a small number. Change the time from t0 to t = t0+dt,
    !!    where dt is a small number.
    !! 3) Run this new input through the control system to get the reaction
    !!    (output) u(e) from the control system due to the change in the input.
    !! 4) Calulate du = u(e)-u(e0), de/dt and int(de)dt
    !!    The objective is to derive:
    !!    du/de, du/(de/dt) and du/int(de)dt.
    !!
    !! Programmer : Magne Bratland
    !! date/rev   : 24 Sept 2009 / 1.0
    !!==========================================================================
    ...
    integer,             intent(in)    :: numVregOut        !! Number of vreg out to read variation from
    integer,             intent(in)    :: whichVregOut(:)   !! Which vreg out to read variation from
    integer,             intent(in)    :: nPerturb          !! number of perturbations on control system
    !                                                       !! if nPerturb = 1, the routine finds
    !                                                       !! only the proportional gains;
    !                                                       !! if nPerturb = 2, the routine finds
    !                                                       !! the proportional and derivative gains;
    !                                                       !! if nPerturb = 3, the routine finds
    !                                                       !! the proportional, derivative and integral gains;
    real(dp),            intent(out) :: ctrlSysGains(:,:,:) !! table for storing control system gains
    !						  		                        !!(no. of outputs from control system,
    !				 				                        !! no. of inputs to control system,
    !		   			   			                        !! no. of perturbations on control system)
    !                                                       !! ctrlSysGains(:,:,1) = Kp
    !                                                       !! ctrlSysGains(:,:,2) = Kd
    !                                                       !! ctrlSysGains(:,:,3) = Ki
    integer            , intent(inout) :: ierr
    ...
    real(dp) :: orgTime                       !! original/initial value for the time
    real(dp) :: orgTimeStep                   !! original/initial value for the time step
!    real(dp) :: vreg0(size(ctrl%vreg))        !! initial values for the control system variables
!    real(dp) :: e0                            !! e0
!    real(dp) :: de0dt                         !! de0/dt
    real(dp), allocatable :: ue0(:)           !! u(e0), output from control system
    !                                         !! when input to control system is e0
    real(dp), allocatable :: ue(:)            !! u(e), output from control system
    !										  !! when input to control system is e = e0+de
    real(dp), allocatable :: du(:)            !! du = u(e)-u(e0)
    real(dp), allocatable :: dt(:)            !! incremental time steps
    !										  !! for use in de/dt and int(de)dt
    real(dp), allocatable :: de(:)            !! incremental step for use in du/de
    real(dp), allocatable :: dedt(:)          !! de/dt, 1st derivative of e
    !										  !! with respect to time t
    real(dp), allocatable :: intdedt(:)       !! integral of e with respect to time t
    real(dp), allocatable :: deMatrix(:,:)    !! matrix of perturbation steps de
    !                                         !! for n perturbations, and
    !										  !! its derivative de/dt and
    !                                         !! integral int(de)dt
    !                                         !! deMatrix(:,1) = de
    !                                         !! deMatrix(:,2) = de/dt
    !                                         !! deMatrix(:,3) = int(de)dt
    real(dp), allocatable :: invDeMatrix(:,:) !! inverse of the de-matrix
    real(dp), allocatable :: duTable(:,:,:)   !! table for storing du-results
    ! 										  !!(no. of outputs from control system,
    ! 										  !! no. of inputs to control system,
    !										  !! no. of perturbations on control system)
    integer :: ctrlSysMode = 3                !! ctrlSysMode = 3 = control system integration
    ...
    i = 0
    j = 0
!    e0 = 0     !! TODO Magne: Get this value from system
!    de0dt = 0  !! TODO Magne: Get this value from system
    if (nPerturb /= size(ctrlSysGains,3)) then
       ierr = -1
       return
    end if

    !! Make copy of control system
    call AllocateCopyControlType(ctrl,ctrlCopy,ierr) !! Allocate and copy
    if ( ierr .ne. 0 ) return

    !! Define length for arrays ue0 and ue
    allocate(ue0(numVregOut),ue(numVregOut),du(numVregOut))
    ue0 = 0.0_dp
    ue = 0.0_dp
    du = 0.0_dp

    !! Define dimensions for the du-table
    allocate(duTable(numVregOut,numVregIn,nPerturb))
    duTable = 0.0_dp
    ...
    !! Save the value of u(e) in an array
    do i = 1, numVregOut
       ue0(i) = ctrlCopy%vreg(whichVregOut(i))
    end do

    !! Establish de-matrix
    allocate(de(nPerturb),dt(nPerturb),dedt(nPerturb),intdedt(nPerturb), &
         &   deMatrix(nPerturb,nPerturb),invDeMatrix(nPerturb,nPerturb))
    do j = 1, nPerturb
       de(j) = orgTimeStep*1.0E-1_dp*j !1.0E-5_dp * j       !! TODO Magne: Change this value to something smaller?
       dt(j) = orgTimeStep*1.0E-1_dp*j !1.0E-5_dp * j       !! TODO Magne: Change this value to something smaller?
       !! Calculate de/dt
       dedt(j) = de(j)/dt(j)             !! TODO Magne: Check this calculation
       !! Calculate int(du)dt
       intdedt(j) = 0.5*de(j)*dt(j)      !! TODO Magne: Check this calculation

       deMatrix(j,1) = de(j)
       if (nPerturb == 2) then
          deMatrix(j,2) = dedt(j)
       else if (nPerturb == 3) then
          deMatrix(j,2) = dedt(j)
          deMatrix(j,3) = intdedt(j)
       end if
    end do

    !! Invert de-matrix
    !call FindInv(deMatrix,invDeMatrix,nPerturb,ErrorFlag)

    !! Start perturbation with time steps dt and input steps de
    do j = 1, nPerturb
       !! The perturbation sequence
       do i = 1, numVregIn
          !! Copy original control system (ctrl) to copy of control system (ctrlCopy)
          call CopyControlType(ctrl,ctrlCopy)
          !! Perturb
          iInput = whichVregIn(i)
          call PerturbController(sys,ctrlCopy,msim,iInput,dt(j),de(j),numVregOut, &
               &                    whichVregOut,ctrlSysMode,ue,ierr)
          if ( ierr .ne. 0 ) return
          !! Calculate du
          du(:) = ue(:)-ue0(:)
          !! Store du-results in a table
    ...
    !! Calculate du/de, du/(de/dt) and du/int(de)dt
    !! [du] = [de]*[du/de]
    !! => [du/de] = inv[de]*[du]
    do j = 1, nPerturb
       do i = 1, numVregIn
          if (nPerturb == 1) then
             ctrlSysGains(:,i,j) = invDeMatrix(j,1)*duTable(:,i,1)
          else if (nPerturb == 2) then
             ctrlSysGains(:,i,j) = invDeMatrix(j,1)*duTable(:,i,1) &
                  &               +invDeMatrix(j,2)*duTable(:,i,2)
          else if (nPerturb == 3) then
             ctrlSysGains(:,i,j) = invDeMatrix(j,1)*duTable(:,i,1) &
                  &               +invDeMatrix(j,2)*duTable(:,i,2) &
                  &               +invDeMatrix(j,3)*duTable(:,i,3)
          else
    ...
    deallocate(ue0,ue,du,dt,de,dedt,intdedt,deMatrix,invDeMatrix,duTable)

  end subroutine EstimateCtrlSysProp_old


subroutine EstimateControllerPropertiesOld(sys,mech,ctrl,msim,      &
    &                                     numVregIn,  whichVregIn,  &
    &                                     numVregOut, whichVregOut,      &
    &                                     nPerturb, ctrlGains, ierr)

    !!==========================================================================
    !! Purpose:
    !! Use a perturbation method, simular to the Matrix Stiffness Method /
    !! (Virtual) Displacement Method / Unit Load Method to find
    !! the properties of the controller. These controller properties
    !! will be added to existing matrices when conducting
    !! modal analysis / eigenvalue analysis.
    !! The values of interest are:
    !! du/dy: Change du in output from controller with respect to
    ...
    integer,             intent(in)    :: whichVregOut(:)   !! Which vreg out to read variation from
    integer,             intent(in)    :: nPerturb          !! number of perturbations on controller
    real(dp),            intent(out)   :: ctrlGains(:,:,:)  !! table for storing controller gains
    !						  		                        !!(no. of outputs from controller,
    !				 				                        !! no. of inputs to controller,
    !		   			   			                        !! no. of perturbations on controller)
    !                                                       !! ctrlGains(:,:,1) = Kp
    ...
    real(dp), allocatable :: du(:)              !! du = u(y)-u(y0)
    real(dp), allocatable :: dt(:)              !! incremental time steps
    !										    !! for use in dy/dt and int(dy)dt
    real(dp), allocatable :: dy(:)              !! incremental step for use in du/dy
    real(dp), allocatable :: dydt(:)            !! dy/dt, 1st derivative of y
    !										    !! with respect to time t
    real(dp), allocatable :: intdydt(:)         !! integral of y with respect to time t
    real(dp), allocatable :: dyMatrix(:,:,:)    !! matrix of perturbation steps dy
    !                                           !! for n perturbations, and
    !                                           !! its derivative dy/dt and
    !                                           !! integral int(dy)dt
    !                                           !! dyMatrix(:,1) = dy
    !                                           !! dyMatrix(:,2) = int(dy)dt
    !                                           !! dyMatrix(:,3) = dy/dt
    real(dp), allocatable :: invDyMatrix(:,:,:) !! inverse of the dy-matrix
    real(dp), allocatable :: duTable(:,:,:)     !! table for storing du-results
    ! 										    !!(no. of outputs from controller,
    ...
    i = 0
    j = 0
    if (nPerturb /= size(ctrlGains,3)) then
       ierr = -1
       return
    end if
    ...
    !! Define length for arrays y0, uy0, uy and du
    allocate(y0(numVregIn),uy0(numVregOut),uy(numVregOut),du(numVregOut))
    y0 = 0.0_dp
    uy0 = 0.0_dp
    uy = 0.0_dp
    ...
    !! Establish dy-matrix
    allocate(dy(nPerturb),dt(nPerturb),intdydt(nPerturb),dydt(nPerturb), &
         &   dyMatrix(nPerturb,nPerturb,numVregIn),invDyMatrix(nPerturb,nPerturb,numVregIn))
    do i = 1, numVregIn
       do j = 1, nPerturb
          dy(j) = orgTimeStep*1.0E-1_dp*j       !! TODO Magne: Change this value?
          dt(j) = orgTimeStep*1.0E-1_dp*j       !! TODO Magne: Change this value?

          !! Calculate int(dy)dt
          intdydt(j) = (y0(i)+0.5*dy(j))*dt(j)  !! TODO Magne: Check this calculation
          !! Calculate dy/dt
          dydt(j) = dy(j)/dt(j)                 !! TODO Magne: Check this calculation

          dyMatrix(j,1,i) = dy(j)
          dyMatrix(j,2,i) = intdydt(j)
          dyMatrix(j,3,i) = dydt(j)
       end do
       !! Invert dy-matrix
       !call FindInv(dyMatrix(:,:,i),invDyMatrix(:,:,i),nPerturb,ErrorFlag)
    end do
    ...
    !! Start perturbation with time steps dt and input steps dy
    do j = 1, nPerturb
       !! The perturbation sequence
       do i = 1, numVregIn
          !! Copy original controller (ctrl) to copy of controller (ctrlCopy)
          call CopyControlType(ctrlCopy,ctrlCopyCopy)
          !! Perturb
          iInput = whichVregIn(i)
          call PerturbController(sys,ctrlCopyCopy,msim,iInput,dt(j),dy(j),numVregOut, &
    ...
    !! Calculate du/dy, du/int(dy)dt and du/(dy/dt)
    !! [du] = [dy]*[du/dy]
    !! => [du/dy] = inv[dy]*[du]
    do j = 1, nPerturb
       do i = 1, numVregIn
          ctrlGains(:,i,j) = invDyMatrix(j,1,i)*duTable(:,i,1) &
               &            +invDyMatrix(j,2,i)*duTable(:,i,2) &
               &            +invDyMatrix(j,3,i)*duTable(:,i,3)
       end do
    end do
    ...
    deallocate(y0,uy0,uy,du,dt,dy,dydt,intdydt,dyMatrix,invDyMatrix,duTable)

  end subroutine EstimateControllerPropertiesOld

    subroutine EstimateCtrlProp_old_01(sys,mech,ctrl,msim,       &
    &                                   numVregIn, whichVregIn,   &
    &                                   numVregOut, whichVregOut, &
    &                                   nPerturb, ctrlProps, ierr)
    ...
    allocate(dt(nPerturb),dy(nPerturb),dintydt(nPerturb),ddydt(nPerturb))
    ...
    do i = 1, numVregIn
       do j = 1, nPerturb
          ...
          dt(j) = orgTimeStep*1.0E-1_dp*j
          dy(j) = dt(j)
          dintydt(j) = (y0(i)+0.5_dp*dy(j))*dt(j)
          ddydt(j) = dy(j)/dt(j)
    ...
    deallocate(y0,uy0,uy,du,dyMatrix,invDyMatrix,duTable,dt,dintydt,ddydt)

  end subroutine EstimateCtrlProp_old_01
